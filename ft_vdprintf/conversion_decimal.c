...
static void	ft_putnbr_intmaxt(t_vdprintf store, intmax_t number)
{
	uintmax_t n;
	...
	if (number < 0)
		...
		n = number;
	if (n >= 10)
		ft_putnbr_intmaxt(store, n / 10);
	ft_putchar_fd(n % 10 + '0', store.fd);
}
...
int			conversion_decimal(t_vdprintf store, intmax_t number)
{
	...
	if (!store.flag_minus && store.flag_zero && store.precision != -1)
		ft_print_character(store, store.spaces, ' ');
	if (store.flag_plus)
		(number >= 0) ? ft_putchar_fd('+', store.fd) :
	...
	if (number != 0 || store.precision != 0)
		(number >= 0) ? ft_putnbr_intmaxt(store, number) :
